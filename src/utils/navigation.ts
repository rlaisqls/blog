...
import type { PrevNextLinkConfig } from '@astrojs/starlight/schemas/prevNextLink';
import type {
    AutoSidebarGroup,
    LinkHTMLAttributes,
    SidebarItem,
...
function configItemToEntry(
    ...
    if ('link' in item) {
        ...
        return {
            ...
            entries: item.items.map((i: any) => configItemToEntry(i, currentPathname, locale, routes)),
...
export function getSidebar(pathname: string, locale: string | undefined): SidebarEntry[] {
    ...
    if (config.sidebar) {
        return config.sidebar.map((group: any) => configItemToEntry(group, pathname, locale, routes));
    } else {
