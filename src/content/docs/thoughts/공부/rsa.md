...
RSA에서 이 mod n의 곱셈 군을 사용한다. 두 소수 p, q의 곱 `n = p × q`를 모듈러스로 선택하면, n과 서로소인 수들이 곱셈 군을 이루고, 암호화와 복호화가 이 군의 성질을 활용한다.
...
왜 하필 4번일까? mod 5에서 곱셈 군의 크기 `φ(5) = 4`이기 때문이다. 어떤 원소든 군의 크기만큼 거듭제곱하면 1로 돌아온다. 이 성질을 암호화에 활용할 수 있다.
