...
원본 가이드는 Debian 12 머신 4대(jumpbox 1, control plane 서버 1, worker 2)를 요구하고 가상화 방식은 자유다. 다만 control plane이 단일 노드라 HA 구성을 살펴보기는 어렵다.

이 글에서는 control plane을 3대로 늘리고 앞단에 LB를 두는 HA 구성으로 확장했다. 본인은 ARM Linux 호스트에서 [KVM](https://www.linux-kvm.org/)/[libvirt](https://libvirt.org/) VM 6대를 띄워 진행했고, 호스트 자체는 jumpbox로 사용한다. 가상화 방식은 본인 환경에 맞게 골라도 될 듯 하다.
...
apiserver를 띄우는 방법은 호스트에서 바이너리를 직접 실행하거나, K8s가 관리하는 Pod으로 띄우는 방식 중에 선택할 수 있다.
...
server URL이 두 그룹으로 갈린다.

- 워커의 kubelet, kube-proxy → `https://lb:6443` (LB 경유)
- cp의 controller-manager, scheduler → `https://127.0.0.1:6443` (자기 노드의 apiserver 직통)

이 분리가 HA의 작동 방식이다. 워커는 cp 한 대가 죽어도 작동해야 하므로 LB를 경유한다. cp 내부 데몬은 같은 노드의 apiserver와 통신하므로 LB를 거칠 이유가 없다.

### TLS Bootstrap

여기까지는 cert가 이미 있는 상황을 가정했다. 새 노드를 추가할 때 kubelet은 자기 cert가 없는 상태로 시작한다. cert가 없으면 apiserver에 mTLS 인증을 할 수 없고, 인증을 할 수 없으면 cert를 받을 방법도 없다. 이 모순을 풀어야 새 노드가 클러스터에 합류할 수 있다.

해결 방식은 크게 두 가지다.

이 실습에서 사용하는 방식은 관리자가 모든 노드의 cert를 미리 발급해두고 노드에 직접 배포하는 방식이다. jumpbox에서 cfssl로 노드별 cert(`CN=system:node:w1`, `O=system:nodes` 등)를 만든 뒤 scp로 워커에 옮긴다. 노드를 추가할 때마다 사람이 같은 작업을 반복한다. CA 개인키를 jumpbox에서만 다루면 되니 노출 면이 좁다.

다른 방법으론 [TLS bootstrap](https://kubernetes.io/docs/reference/access-authn-authz/kubelet-tls-bootstrapping/)이 있다. 새 노드가 단기 토큰으로 임시 인증을 통과한 뒤 자기 cert를 클러스터에 요청해 받아오는 방식이라, 노드를 동적으로 추가하는 상황에 유리하다. 흐름은 다음과 같다
...
3대를 거의 동시에 시작해야 quorum이 형성된다. 한 대만 먼저 켜면 나머지 둘을 기다리다 timeout된다. 검증은 `etcdctl member list`로 한다. 3개 멤버가 보이면 etcd가 정상이다.
...
- **백업**: `etcdctl snapshot save`로 주기적으로 스냅샷을 떠둬야 한다. etcd 멤버 3대가 동시에 손상되면 클러스터 전체 상태가 복구 불가능하다.
...
컨트롤 플레인의 핵심이다. K8s에서 일어나는 모든 변화가 apiserver를 거친다.
...
systemd unit에 25개가 넘는 플래그가 붙는다. 각 플래그가 K8s 설계의 한 단면을 보여준다.

#### 6단계 파이프라인

요청이 들어오면 다음 6단계 파이프라인을 거친다.

1. **Authentication**: 요청자 신원 확인
2. **Authorization**: 요청 권한 검사
3. **Mutating Admission**: 객체 변형
4. **Validating Admission**: 객체 검증
5. **etcd write**: 저장
6. **Response**: 응답

각 단계의 동작을 좀 더 풀면 다음과 같다.

- **Authentication**: 들어온 요청이 누구인지 식별한다. 인증 수단이 여러 개 있고 [순서대로 시도한다](https://github.com/kubernetes/kubernetes/blob/master/staging/src/k8s.io/apiserver/pkg/authentication/request/union/union.go).
  - **client cert**: TLS handshake에서 받은 cert의 CN/O를 사용자/그룹으로 인식 (앞서 [Subject와 신원](#subject와-신원)에서 본 그 동작).
  - **service account 토큰**: Pod 안에서 들어온 요청의 `Authorization: Bearer ...` JWT를 [검증](https://github.com/kubernetes/kubernetes/tree/master/pkg/serviceaccount).
  - **bootstrap 토큰**: 노드 합류용 단기 토큰. [구현](https://github.com/kubernetes/kubernetes/tree/master/staging/src/k8s.io/cluster-bootstrap/token).
  - **OIDC**: 외부 IdP(Google, Keycloak 등) 연동. [구현](https://github.com/kubernetes/kubernetes/tree/master/staging/src/k8s.io/apiserver/plugin/pkg/authenticator/token/oidc).
- **Authorization**: 식별된 신원이 이 요청을 할 권한이 있는지 검사한다. 여러 인가기를 체인으로 묶고 하나라도 허용하면 통과(`--authorization-mode=Node,RBAC` 식; [union 인가기](https://github.com/kubernetes/kubernetes/blob/master/staging/src/k8s.io/apiserver/pkg/authorization/union/union.go)). [Node Authorizer](https://github.com/kubernetes/kubernetes/tree/master/plugin/pkg/auth/authorizer/node)는 kubelet의 권한을 자기 노드 자원으로 좁히고, [RBAC authorizer](https://github.com/kubernetes/kubernetes/tree/master/plugin/pkg/auth/authorizer/rbac)는 그 외 일반 사용자/SA의 권한을 ClusterRole/RoleBinding으로 평가한다.
- **Mutating Admission**: 객체를 etcd에 쓰기 전에 변형한다. 빌트인 예시로 [`ServiceAccount` 어드미션](https://github.com/kubernetes/kubernetes/tree/master/plugin/pkg/admission/serviceaccount)이 Pod에 SA 토큰 볼륨을 자동 마운트하고, [`DefaultStorageClass`](https://github.com/kubernetes/kubernetes/tree/master/plugin/pkg/admission/storage/storageclass/setdefault)가 PVC에 default StorageClass를 박는다. 외부 [`MutatingWebhookConfiguration`](https://github.com/kubernetes/kubernetes/tree/master/staging/src/k8s.io/apiserver/pkg/admission/plugin/webhook/mutating)도 이 단계에 끼어든다. Istio 사이드카 주입이 대표적이다.
- **Validating Admission**: 변형 끝난 객체가 정책을 만족하는지 본다. [`ResourceQuota`](https://github.com/kubernetes/kubernetes/tree/master/plugin/pkg/admission/resourcequota), [`PodSecurity`](https://github.com/kubernetes/kubernetes/tree/master/staging/src/k8s.io/pod-security-admission), [OPA Gatekeeper](https://open-policy-agent.github.io/gatekeeper/), [Kyverno](https://kyverno.io/) 같은 외부 정책 엔진도 이 단계에 [webhook](https://github.com/kubernetes/kubernetes/tree/master/staging/src/k8s.io/apiserver/pkg/admission/plugin/webhook/validating)으로 들어간다. 여기서 거부되면 etcd에 쓰지 않고 바로 4xx 응답이 나간다.
- **etcd write**: 직렬화한 객체를 etcd에 저장한다. 이 단계 직전에 strategic merge patch / optimistic concurrency(`resourceVersion` 비교) 같은 충돌 처리가 들어간다.
- **Response**: 클라이언트에 결과를 반환한다. `kubectl create`가 끝나는 지점이 여기다. 이 시점에 실제 컨테이너는 아직 떠 있지 않다. 컨테이너 생성은 controller/scheduler/kubelet이 watch로 받아 비동기로 처리한다.
...
--client-ca-file=ca.pem
   들어온 client cert(admin/kubelet/...)가 우리 CA로 서명됐는지 검증한다

--service-account-key-file=service-account.pem
   들어온 SA 토큰(JWT)을 공개키로 검증한다

--service-account-signing-key-file=service-account-key.pem
   새 SA 토큰 발급 시 개인키로 서명한다
```

`--service-account-key-file`은 이름에 "key"가 들어 있지만 실제로는 공개키가 들어 있는 cert를 가리킨다. 옛 명명이 그대로 남아 있는 경우다. 서명은 개인키로, 검증은 공개키로 한다는 비대칭 암호의 기본 규칙은 변하지 않는다.

#### 그 외 알아두면 좋은 기능

- **API aggregation**: `metrics-server`나 사용자 정의 API 서버를 메인 apiserver 뒤에 붙일 수 있다. apiserver가 들어온 요청을 보고 해당 API group이 등록된 외부 서버로 프록시한다.
- **Audit log**: 정책 파일로 어떤 요청을 어떤 수준으로 기록할지 정한다. 보안 사고 분석의 근거가 된다.
- **OpenAPI/Discovery**: apiserver가 자기 API 스키마를 직접 publish한다. `kubectl explain`이나 클라이언트의 동적 타입 처리가 이를 사용한다.

### controller-manager, scheduler

apiserver는 들어온 요청을 받아 etcd에 반영하는 게이트일 뿐이다. "Pod이 부족하면 만들고", "노드가 죽으면 정리하는" 식의 실제 의사결정은 다른 컴포넌트가 한다.
...
코드 흐름은 대략 이렇게 생겼다 (실제 빌트인 컨트롤러도 거의 이 모양이다).
...
[scheduler](https://github.com/kubernetes/kubernetes/tree/master/pkg/scheduler)는 Pod이 만들어졌는데 `nodeName`이 비어 있을 때(아직 띄울 노드가 정해지지 않았을 때) 노드를 결정한다. 동작은 두 단계로 나뉜다.
...
`mode tcp` (L4 passthrough)를 쓴다. LB가 TLS를 풀지 않고 그대로 전달한다.

LB가 TLS termination을 하면 LB가 자기 cert를 클라이언트에 제시하게 된다. 클라이언트는 LB의 신원을 보게 되어 mTLS의 client cert 인증이 깨진다 (apiserver에 전달되어야 할 client cert 자체가 LB에서 풀려 사라진다). 그래서 LB는 TCP 라우터로만 동작하고 TLS 협상은 양 끝(client ↔ apiserver)에서 직접 한다.
...
kube-proxy가 만드는 iptables 룰을 `iptables-save -t nat`으로 보면 다음 패턴이 반복된다 (체인 이름은 단축).
...
순서가 핵심이다. `KUBE-SERVICES`는 모든 ClusterIP에 대한 매칭 룰이 들어 있는 디스패처고, `KUBE-SVC-XXX`는 백엔드 후보 중에서 하나를 고르는 분배기, `KUBE-SEP-XXX`는 실제 DNAT을 적용하는 종착 체인이다. Endpoint가 N개면 `KUBE-SVC` 체인에 N개의 분기 룰이 들어가고 마지막 한 개만 `--probability` 없이 fallback으로 박힌다.
...
kubelet을 시작하기 전에 리눅스 호스트에 다음 작업들을 해야 한다. 하나라도 빠지면 K8s가 시작을 거부하거나, 더 곤란한 경우엔 일단 뜨고 나서 미묘한 버그를 만든다.
...
지금까지는 IP만으로 통신이 가능한 상태다. 사용자가 이름으로 Service에 접근하려면 클러스터 내부 DNS가 필요하다.

[CoreDNS](https://coredns.io/)는 K8s 내부에서 Pod 형태로 동작하는 DNS 서버다. apiserver를 watch하며 Service 목록을 유지하다가 DNS 쿼리에 응답한다.
...
**Control Plane (cp 노드 위 systemd 데몬)**

- kube-apiserver (REST API + 인증/인가/admission/etcd write 파이프라인)
- kube-controller-manager (reconcile loop)
- kube-scheduler (Pod 배치 알고리즘)
- HAProxy on lb (L4 passthrough HA)

**Worker (worker 노드 위 systemd 데몬)**

- containerd (CRI 런타임)
- runc (OCI 런타임, 실제 namespace/cgroup)
- kubelet (apiserver↔CRI 중재자)
- kube-proxy (iptables 룰 작성)

**클러스터 안 Pod**

- CoreDNS (Service 이름 해결)
...
- PKI (CA 1개로 모든 cert 서명, mTLS 양방향)
- kubeconfig (cert + 서버 URL + 컨텍스트)
- RBAC (system:masters, system:nodes 등 그룹으로 권한 매핑, 빌트인 다수)

**Networking**

- L3 라우팅 (Pod 간 직접 IP)
- L4 NAT (kube-proxy iptables, Service ClusterIP)
- L7 (CoreDNS)

이걸 한 줄(`kubeadm init`)로 끝내면 kubeadm이다. 운영 부담은 적지만 추상화 층이 깊다. 하나하나 손으로 짜면 The Hard Way다. 운영 손이 많이 가지만 내부 동작이 그대로 드러난다.

The Hard Way를 한 번 거치면 이후 다른 K8s 클러스터를 다룰 때 전체 구조가 보인다.
...
- [본 글의 실습 코드](https://github.com/rlaisqls/play-with-linux/tree/main/kubernetes)
