...
인터넷에서 메시지나 파일을 주고받을 때 세 가지 문제가 있다.
...
GPG는 이 세 가지 문제를 공개 키 암호화와 디지털 서명으로 해결한다.
...
예를 들어 Alice가 "Bob의 공개 키"라고 주장하는 키를 받았다고 하자. 하지만 이게 진짜 Bob의 키일까? 중간에 공격자가 자신의 키를 Bob의 키라고 속일 수도 있다. (중간자 공격, MITM)
