...
- ERC20(Ethereum Request for Comment 20)
  ...
  - ERC20의 보완 버전 -> ERC223, ERC777, 하지만 Fallback 함수를 지원해 재진입 공격의 위험이 있고 더 무거움. 아직 ERC20이 선호됨
...
- Deposit contract(예치 콘트랙트)
  - 비콘 체인의 균형을 잡을 수 있는 콘트랙트이다. 이더리움 1.0 네트워크에 존재한다.
  - 콘트랙트의 ETH는 일단 입금되면 1.0 sp트워크에 쓸 수 없다. 검증자가 되기 위해 필요한 최소 보증금은 32ETH이다.
  - 대부분의 증거 시스템과 마찬가지로, 검증자 역할을 한 것에 대한 재정적 보상이 있다.

- 정직한 검증자 프레임워크
  - ethereum 2.0 네트어크를 안전하게 유지하기 위해 검증자들이 따라야 하는 일련의 기준이 있다. 여기에는 제안된 블록에 서명하고 투표를 위해 사용 가능한 개인 키와 활성 검증자가 되어 생성된 자금을 회수하기 위한 별도의 개인키가 포함돼 있는데, 이는 오프라인에 안전하게 저장되어야 한다.
...
- 이더리움 포크 체인 중 하나인 Gnosis Chaind은 GNO 토큰과 xDAI 토큰을 함께 사용한다.
- 가장 큰 차이점은 GNO은 네트워크의 거버넌스 및 스테이킹(보안)에 사용되는 토큰이고, xDAI는 해당 체인 내의 거래 수수료(가스비)를 지불하는 데 사용되는 스테이블 코인이라는 점이다.
  - <https://docs.gnosischain.com/about/tokens>

- GNO (Gnosis) Token
  - Gnosis Chain은 지분증명(PoS) 방식으로 운영된다. 네트워크 검증자(Validator)들은 GNO 토큰을 스테이킹(예치)하여 네트워크를 보호하고 블록을 생성하며, 그 대가로 보상을 받는다.
  - GNO 토큰 보유자들은 GnosisDAO에 참여하여 Gnosis 생태계의 업그레이드나 자금 사용처 등 중요한 안건에 대해 투표할 수 있다.
  - GNO의 가격은 시장에 따라 변동한다.
  - <https://docs.gnosischain.com/about/tokens/gno>

- xDAI Token
  - xDAI는 Gnosis Chain의 네이티브 토큰(Native Token)으로, 네트워크 내에서 발생하는 모든 거래의 가스비를 지불하는 데 사용된다.
  - USD에 대해 Stable이다. 1 xDAI ≈ 1 DAI ≈ $1 이다.
  - Gnosis Chain에서 트랜잭션을 실행하거나 스마트 컨트랙트를 배포할 때 발생하는 수수료(가스비)는 xDAI로 지불한다.
  - 가스비를 변동성 자산(ETH 등)이 아닌 스테이블 코인(xDAI)으로 지불하기 때문에, 사용자는 예측 가능한(안정적인) 수수료로 네트워크를 이용할 수 있다.
