---
title: "sort"
lastUpdated: 2026-03-15T22:59:19
---
Python의 정렬은 `sorted()`와 `list.sort()`로 수행한다. 정렬 기준을 지정하는 방식이 Python 2와 3에서 다르다.

## cmp

Python 2 스타일의 비교 함수이다ㅓ. 두 원소를 받아서 비교 결과를 반환한다. Python 3에서는 제거되었다.
...
Python 3 스타일의 키 함수이다. 각 원소를 받아서 비교용 값을 반환한다.

```python
sorted([3, 1, 2], key=lambda x: -x)
```

Python 3에서 `cmp`가 제거되고 `key`만 남은 이유는 다음과 같다

- key는 원소당 1번만 호출(O(n)), cmp는 비교마다 호출(O(n log n))하므로 key 함수의 성능이 더 좋다
- 게다가 key 함수가 이해하기 더 쉽다
