...
lastUpdated: 2026-03-16T04:09:13
---
...
**`ProxyType` vs `CallableProxyType`**

`weakref.proxy()`가 반환하는 프록시의 구체 타입은 두 가지인데, 감싸는 대상이 호출 가능한지에 따라 자동으로 결정된다.
...
type(weakref.proxy(Data()))  # <class 'weakref.ProxyType'>
type(weakref.proxy(f))       # <class 'weakref.CallableProxyType'>
```

- **`ProxyType`**: 일반 객체용. 속성 접근 등은 위임하지만 프록시 자체를 호출할 수는 없다.
- **`CallableProxyType`**: 대상이 함수·메서드거나 `__call__`을 가진 객체일 때 반환된다. `ProxyType`에 `__call__` 위임만 추가한 타입으로, 프록시를 직접 호출하면 원본이 호출된다.

```python
def greet(name):
    return f"hello {name}"

p = weakref.proxy(greet)
p("world")  # "hello world" — 호출이 원본 함수로 위임됨
```

타입을 둘로 나눈 이유는 `callable()` 검사 때문이다. `callable(x)`는 인스턴스가 아니라 **타입에 `__call__`(C 레벨의 `tp_call` 슬롯)이 있는지**를 본다. 프록시 타입이 하나뿐이고 거기에 `__call__`이 구현되어 있다면, 호출 불가능한 일반 객체를 감싼 프록시도 `callable()` 검사를 통과해버려 원본과 다르게 동작한다. 대상의 callable 여부에 따라 `__call__`이 있는 타입과 없는 타입을 골라줌으로써 `callable()` 결과가 원본과 일치하게 만든 것이다.

```python
callable(Data())                 # False
callable(weakref.proxy(Data()))  # False — ProxyType에는 __call__이 없음

callable(f)                      # True
callable(weakref.proxy(f))       # True  — CallableProxyType
```

`CallableProxyType`을 직접 생성할 일은 없고 `weakref.proxy()`를 쓰면 된다. 콜백을 약한 참조로 보관하는 observer 패턴에서 유용한데, 원본 콜백이 GC된 뒤 호출하면 `ReferenceError`가 발생하므로 `try/except ReferenceError`로 죽은 콜백을 정리하는 코드가 따라붙는 게 보통이다.

```python
class Button:
    def __init__(self, callback):
        # 콜백 객체의 수명을 버튼이 연장하지 않도록 프록시로 보관
        self._callback = weakref.proxy(callback)

    def click(self):
        self._callback()  # 원본이 살아있으면 호출, 죽었으면 ReferenceError
```
